...
import {getAwaitingGames} from '../AwaitingGames/index'
...
async function addToAwaitList(name, date) {
	console.log('adding to list')
	return await getAwaitingGames().then(x=>
		console.log(x))
}
		...
		if (isDuplicate === true) {
			...
			return game.set(gameModel, async (error) => {
				if (error) {
					...
					await addToAwaitList(data.name,data.date)
					return res.json({
